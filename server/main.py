"""wolfgang_rush — Motor Accident Claims Tribunal Drafting MCPB.

Local-execution MCPB Desktop Extension for the MV Act 1988 (2019 amended)
MACT architecture across India. 10 case types + 8 State exemplars.

Privacy: zero data collection.
License: MIT
Source: https://github.com/Wolfgangrush/indian-mact-drafting-mcpb
...
mcp = FastMCP("wolfgang-indian-mact-drafting")

BUNDLE_ROOT = Path(__file__).resolve().parent.parent
SKILLS_DIR = BUNDLE_ROOT / "skills"
AGENTS_DIR = BUNDLE_ROOT / "agents"
PLEADING_BASE_DIR = SKILLS_DIR / "_mact_drafting_base"
STATE_CONFIG_DIR = BUNDLE_ROOT / "state-config" / "exemplars"

CASE_TYPES: list[str] = [
    "mact-petition-section-166-fault-based",
    "mact-petition-section-164-lump-sum",
    "mact-petition-section-161-solatium-hit-and-run",
    "mact-petition-third-party-insurance-section-147",
    "mact-appeal-section-173-to-high-court",
    "mact-interim-compensation-application",
    "mact-claim-execution-application",
    "compensation-recovery-collector-section-174",
    "motor-vehicle-criminal-defence-bnss",
    "workmen-compensation-claim",
]

CASE_TYPE_DESCRIPTIONS: dict[str, str] = {
    "mact-petition-section-166-fault-based": "Fault-based compensation petition under Section 166 MV Act 1988",
    "mact-petition-section-164-lump-sum": "Lump-sum no-fault claim under Section 164 MV Act (2019 amendment)",
    "mact-petition-section-161-solatium-hit-and-run": "Solatium and hit-and-run claim under Section 161 MV Act + Solatium Fund Scheme",
    "mact-petition-third-party-insurance-section-147": "Third-party insurance claim under Section 147 MV Act",
    "mact-appeal-section-173-to-high-court": "Appeal to High Court under Section 173 MV Act",
    "mact-interim-compensation-application": "Interim compensation application before MACT",
    "mact-claim-execution-application": "Execution application for unpaid MACT award",
    "compensation-recovery-collector-section-174": "Recovery from Collector as land revenue arrears under Section 174",
    "motor-vehicle-criminal-defence-bnss": "Criminal defence in MV Act prosecution under BNSS 2023",
    "workmen-compensation-claim": "Workmen's Compensation claim under Employee's Compensation Act 1923",
}
...
def _list_states_internal() -> list[str]:
    ...
    return sorted(
        p.stem
        for p in STATE_CONFIG_DIR.iterdir()
...
FULL_ORCHESTRATION_SCRIPT = """# wolfgang_rush — INDIAN MOTOR ACCIDENT CLAIMS TRIBUNALS DRAFTING · FULL ORCHESTRATION SCRIPT
...
mact-petition-section-166-fault-based, mact-petition-section-164-lump-sum, mact-petition-section-161-solatium-hit-and-run, mact-petition-third-party-insurance-section-147, mact-appeal-section-173-to-high-court, mact-interim-compensation-application, mact-claim-execution-application, compensation-recovery-collector-section-174, motor-vehicle-criminal-defence-bnss, workmen-compensation-claim.
...
def get_pleading_base() -> str:
    """Get the shared MACT drafting base structure (includes Sarla Verma + Pranay Sethi multiplier doctrine)."""
...
def list_states() -> dict:
    """List the State exemplars available for MACT drafting.

    Returns 8 priority State exemplars covering solatium-fund variations,
    State Government notifications, and State High Court procedural notes.
    """
    states = _list_states_internal()
    return {"states": states, "count": len(states), "note": "Call get_state_exemplar(state) for the per-State content. State names use lowercase hyphenated form."}
...
def get_state_exemplar(state: str) -> str:
    """Get the State-specific exemplar for MACT in a given State.
